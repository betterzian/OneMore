...
def run(file_name,cpu_request):
    ...
    np.savetxt('/disk7T/vis/code/OneMore/srcData/online_task/' + str(file_name) + '.csv',chunk, delimiter=",")
    chunk = pd.DataFrame(chunk).transpose()
    chunk.to_csv('/disk7T/vis/code/OneMore/srcData/other/container.txt', index=False, header=0, sep='\t', mode='a',encoding="utf-8", quoting=csv.QUOTE_NONE, escapechar=',')
    return

if __name__ == '__main__':
    meta = pd.read_csv('../srcData/other/container_meta.csv', header=None,
    ...
    p.join()
    pbar.close()
    chunk = pd.read_csv('/disk7T/vis/code/OneMore/srcData/other/container.txt', header=None, sep='\t', encoding="utf-8",
                        quoting=csv.QUOTE_NONE, escapechar=',')
    chunk = chunk.values.astype(np.float64)
    np.savetxt('/disk7T/vis/code/OneMore/srcData/other/container.csv', chunk, delimiter=",")
