...
class BufferArray:
    def __init__(self, memo_max_len):
        self.__state = np.ones((memo_max_len, 9), dtype=np.float32) * -1
        self.__state_len = np.ones((memo_max_len, 100)).astype(int) * -1
        self.__next_state = np.ones((memo_max_len, 100, 8, 9), dtype=np.float32) * -1
    ...
    def add_memo(self, state, next_state, state_len, state_value):
        self.__state[self.__next_idx] = state.reshape(-1,9)
        self.__state_len[self.__next_idx] = state_len
        self.__next_state[self.__next_idx] = next_state
...
class TrainBot():
    def __init__(self,lr = 0.001,epoch = 2000):
        ...
        self.__task_list = np.loadtxt("../srcData/offline_task/off_task_list.csv", delimiter=',', dtype=float)
        ...
        self.__writer = SummaryWriter("../log/model")
        ...
        for i in range(self.__epoch):
            ...
            for count in range(1,2001):
                ...
                if count % 500 == 0:
                    ...
                    turn += 1
            pbar.update(1)
            torch.save(self.__state_value.state_dict(), '../srcData/offline_task/model/off_task_list_model.pth')
        pbar.close()
        torch.save(self.__state_value.state_dict(), '../srcData/offline_task/model/off_task_list_model.pth')
    ...
    def __generate_state(self, i, cpu_max=129, gpu_max=10):
        half_state = np.array([64,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5])
