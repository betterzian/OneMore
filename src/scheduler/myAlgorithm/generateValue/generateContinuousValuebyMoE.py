...
class TrainBot():
    def __init__(self,filename,lr = 0.0005):
        self.__lr = lr
    ...
    def train_sin_expert(self):
        pbar = tqdm(desc=f"{self.__filename} epoch")
        turn = 0
    ...
    def __get_cgr(self,filename):
        ...
        return offline_data_process(filename,only_cgr=True)
...
def run(args):
    ...
    np.random.seed(seed)
    train_bot = TrainBot(args.filename)
    train_bot.train_sin_expert()
    train_bot.train()
